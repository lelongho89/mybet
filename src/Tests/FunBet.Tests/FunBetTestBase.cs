...
namespace FunBet.Tests
{
    public abstract class FunBetTestBase : AbpIntegratedTestBase<FunBetTestModule>
    {
        ...
        protected void UsingDbContext(Action<FunBetDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FunBetDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FunBetDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FunBetDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FunBetDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FunBetDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FunBetDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FunBetDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FunBetDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FunBetDbContext, Task<T>> func)
        {
            T result;
